function x = processdata2025(foldername, repeats)
%ERHAN 2025: This is modifed from show_stats_disc_succnorm_sem.m which
% generates the plots for the paper from a given data fodler. PLOS paper 
% data folder is shown by show_stats_disc_succnorm_sem('keep1_REPS2021_4_21_15_0',60)
% Now, it also compares the data with human data
    N = 84;
    COMP=1; SAFETY=2; FALL=3;
    if ~exist('foldername','var')
        foldername = 'keep1_REPS2021_4_21_15_0';repeats = 60;
    ...
    for k_it=1:repeats
       ...
       % Need to run the simulation again to fix this properly. 
        TA_M(k,:)   = exper.h_TA(rix); negfallix = TA_M(k,:)<-400;  TA_M(k,negfallix) = -400; % ERH2025 This is done to be in complaince with experimental procedures
    ...
    figg = 146;    % ERH2025: figg=46 uses the zero at the end of TA_M etc. small bug. figg~=46 fixes this. The end results is not very different.
    if figg==146
        TA_Mo  = TA_M(1:OEC,:);
        ITA_Mo = ITA_M(1:OEC,:);
        cost_Mo = cost_M(1:OEC,:);
    else
        figg = 46;
        TA_Mo  = TA_M;
        ITA_Mo = ITA_M;
    ...
    plot_mean_std_nan(ntr, mean(TA_Mo), sem(TA_Mo), allcol,whiten(allcol,0.5),3);hold on;
    ...
    plot_mean_std_nan(ntr, mean(ITA_Mo), sem(ITA_Mo), allcol,whiten(allcol,0.5),3); hold on;
    ...
    plot_mean_std_nan(ntr, mean(cost_Mo), sem(cost_Mo), allcol,whiten(allcol,0.5),3); hold on;
    ...
    %************** WE NOW COMPARE WITH  HUMAN DATA ********
    %*******************************************************
    %selfcorr(ITA_Mo, TA_Mo, 4)
    fprintf("Loading human experiment data...\n");
    [ITA_hum, TA_hum] = getExpData;
    selfcorr(ITA_hum, TA_hum, 5);
    fprintf("Computing correlation between SIM and EXP for TA...\n")
    crosscorr(TA_Mo', TA_hum, 1, 'TA');
    crosscorr(ITA_Mo', ITA_hum, 3, 'ITA');
